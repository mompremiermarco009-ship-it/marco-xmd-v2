...
module.exports = {
    ...
    category: 'group',
    desc: 'Promeut un membre en admin du groupe',
    usage: '.promote @user / .promote <numéro>',
    ...
    async execute(sock, msg, args) {
        ...
        try {
            ...
            if (targetP.admin === 'admin' || targetP.admin === 'superadmin') {
                return sock.sendMessage(jid, { text: '❌ Déjà administrateur.' }, { quoted: msg });
            }
            ...
            const text = `╔════════════════════════╗\n` +
                         `║   ⬆️  𝐏𝐫𝐨𝐦𝐨𝐭𝐢𝐨𝐧\n` +
                         `╚════════════════════════╝\n\n` +
                         `┃  ⬆️  @${normalizeNumber(target)} est maintenant admin.\n\n` +
                         `> 𝑃𝑜𝑤𝑒𝑟𝑒𝑑 𝑏𝑦 ${owner}`;

            await sock.sendMessage(jid, { text, mentions: [target] }, { quoted: msg });
        } catch (err) {
            console.error('Erreur promote:', err.message);
            await sock.sendMessage(jid, { text: '⚠️ Erreur lors de la promotion.' }, { quoted: msg });
        }
    }
};
